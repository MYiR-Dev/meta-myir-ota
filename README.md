...
## Overview
- This layer is used to demonstrate SW update OTA use case on STM32MP1 boards.
...
- Look at [Overall presentation](https://github.com/PRG-MPU-CUST/meta-st-ota/blob/mickledore/files/doc/FOTA-MMDV-5.0.0-v1.1.pdf).
...
## 1. Documentation
- [STM32MP1-ecosystem-v5.0.0 Release note](https://wiki.st.com/stm32mpu/wiki/STM32_MPU_OpenSTLinux_release_note_-_v5.0.0)
...
## 2. HW requirements
A STM32MP135F-DK or STM32MP157F-DK2 or STM32MP157F-EV1 is requested.
...
### Install the [OpenSTLinux distribution](https://wiki.st.com/stm32mpu/wiki/STM32MP1_Distribution_Package#Installing_the_OpenSTLinux_distribution)

### Replace [Initializing the OpenEmbedded build environment chapter](https://wiki.st.com/stm32mpu/wiki/STM32MP1_Distribution_Package#Initializing_the_OpenEmbedded_build_environment) content by the hereafter explanations:
...
Waiting a correction, here is the command to workaround the issue:
```
cd <Yocto source tree>/meta-st/meta-st-stm32mp/conf/eula/
ln -s ST_EULA_SLA stm32mp1-ota
```

Execute the following commands to configure environement with the new machine for OTA.
```
cd <Yocto source tree>/build-openstlinuxweston-stm32mp1-ota/
...
The layer meta-st-ota contains a machine named "stm32mp1-ota" that needs to be used when sourcing the environment:
```
DISTRO=openstlinux-weston MACHINE=stm32mp1-ota source layers/meta-st/scripts/envsetup.sh
```
Then you can [source your environment and build the baseline](https://wiki.st.com/stm32mpu/wiki/STM32MP1_Distribution_Package#Building_the_OpenSTLinux_distribution).
After that, you can [flash the built image](https://wiki.st.com/stm32mpu/wiki/STM32MP1_Distribution_Package#Flashing_the_built_image).
...
- tf-a : the following log is displayed  'INFO:    Use FIP_A partition' if "A" partition is used (FIP_B in case of B partition)
